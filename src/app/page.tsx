...
export default function Home() {
  ...
  const { mutate: submitCv } = trpcApi.cv.submit.useMutation({
    ...
    onError: (error) => {
      ...
      setIsSubmitting(false);
      setStatus("FAILED");
      setMismatches({ error: "Failed to upload your CV. Please check your internet connection and try again." });
    },
  });
  ...
  function formatUserFriendlyError(mismatches: Record<string, unknown> | null): string {
    ...
    if (mismatches.experience) {
      errors.push(`• Your experience description doesn't match your CV content`);
    }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (file.size > 5 * 1024 * 1024) {
      ...
      setStatus("FAILED");
      setMismatches({ error: `File size (${sizeMB}MB) exceeds the 5MB limit. Please choose a smaller PDF file.` });
      return;
    }
    ...
    if (!file.type.includes('pdf')) {
      setStatus("FAILED");
      setMismatches({ error: "Please upload a PDF file only. Other file types are not supported." });
      return;
    }
    ...
    reader.onload = async () => {
      ...
      console.log("Is HTMLFormElement:", formElement instanceof HTMLFormElement);
        ...
        >
          ...
          <label className="flex flex-col gap-1">
            ...
            <input
              ...
              className="rounded bg-white/20 px-3 py-2 text-white file:bg-white/20 file:text-white file:border-0 file:rounded file:mr-2"
            />
            {fileName && (
              <span className="mt-1 text-xs text-green-200 flex items-center gap-1">
          ...
          <button
            ...
            className={`rounded-full px-10 py-3 font-semibold transition flex items-center justify-center gap-2 ${
          ...
          {(isSubmitting || status === "PROCESSING") && (
            <p className="text-center text-sm text-white/70 mt-2">
              {isSubmitting ? "Uploading your CV..." : "Please wait while we validate your CV"}
            </p>
          )}
        </form>
          ...
          >
            {status === "PROCESSING" && (
              <div className="text-center text-blue-200">
                ...
                <p className="font-semibold text-lg mb-2">Processing Your CV</p>
                <p className="text-sm opacity-90">{getProcessingMessage()}</p>
                <p className="text-xs mt-3 opacity-75">This usually takes 30-60 seconds</p>
              </div>
            )}
            ...
            {status === "SUCCESS" && (
              <div className="text-center text-green-200">
                ...
                <p className="font-bold text-lg mb-2">✅ CV Successfully Verified!</p>
                <p className="text-sm opacity-90">Your CV matches all the information you provided. Great job!</p>
                <button
                  onClick={() => {
                    setStatus(null);
                    setSubmissionId(null);
            ...
            {status === "FAILED" && (
              <div className="text-red-200">
                ...
                <p className="font-bold text-lg mb-3 text-center">❌ Validation Failed</p>
